...
#pragma mark - 更多城市三个Label
    ...
    for (int i = 0; i < 3; i++) {
        ...
        [button addTarget:self action:@selector(buttonActionbutton:) forControlEvents:UIControlEventTouchUpInside];
        button.tag = 1500 + i;
...
- (void)buttonActionbutton:(UIButton *)button {
    NSArray *recommend_cityArray = [_dataDic objectForKey:@"recommend_city"];
    for (int i = 0; i < 3; i++) {
        
        if (button.tag - 1500 == i) {
            NSDictionary *recommend_cityDic = recommend_cityArray[i];
            [self.delegate AZ_deliverButtonCityID:[recommend_cityDic objectForKey:@"id"]];
        }
    }
}
